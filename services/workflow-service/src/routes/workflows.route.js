...
router.post("/", serviceAuth, checkPermission("workflow.submit"), async (req, res, next) => {
  try {
...
router.get("/", serviceAuth, checkPermission("workflow.submit"), async (req, res, next) => {
  try {
...
router.post("/submissions", serviceAuth, checkPermission("workflow.submit"), async (req, res, next) => {
  try {
...
router.get("/submissions/:submissionId", serviceAuth, checkPermission("workflow.submit"), async (req, res, next) => {
  try {
    const submission = await FormSubmission.findOne({ _id: req.params.submissionId, tenantId: req.tenantId });
    if (!submission) {
      throw new ApiError(404, "Submission not found");
