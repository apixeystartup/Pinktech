...
const AGREEMENT_ALLOWED = /<\/?(?:h1|h2|h3|h4|p|ul|ol|li|strong|em|br|hr|blockquote)\b[^>]*>/gi;
...
function sanitizeAgreementHtml(input) {
  return String(input ?? "")
    ...
    .replace(/<\/?([a-z0-9]+)(\s[^>]*)?>/gi, (match, tag) =>
      AGREEMENT_ALLOWED.test(match) ? match : ""
    );
}
...
<html>
  ...
  <body>
    ...
    <div class="section agreementBox">
      ...
      <div>${sanitizeAgreementHtml(agreements.pre?.text)}</div>
    ...
    <div class="section agreementBox">
      ...
      <div>${sanitizeAgreementHtml(agreements.post?.text)}</div>
