...
const PinkFormAuditLog = require("../models/pinkFormAuditLog.model");
...
function submissionsPublicPath() {
  return process.env.PINK_FORM_SUBMISSIONS_PATH || "/api/v1/submissions";
}
...
router.get("/:submissionId/pdf", async (req, res, next) => {
  try {
    ...
    const fallbackPath = path.join(process.cwd(), "storage", "pdfs", `submission-${req.params.submissionId}.pdf`);
    if (fs.existsSync(fallbackPath)) {
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="submission-${req.params.submissionId}.pdf"`
      );
