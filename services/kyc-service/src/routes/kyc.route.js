...
/** KYC console: prefer `kyc.manage`; `workflow.submit` kept for existing roles. */
const KYC_STAFF_PERMISSIONS = ["kyc.manage", "workflow.submit"];
...
router.post("/external-users", async (req, res, next) => {
  try {
...
router.get("/external-users", async (req, res, next) => {
  try {
    ...
    const canViewAll = permissionCodes.includes("*") || permissionCodes.includes("tenant.manage");
...
router.patch("/external-users/:externalUserId", async (req, res, next) => {
  try {
...
router.post("/initiate", async (req, res, next) => {
  try {
    ...
    await notificationAdapter.sendEmail({
      ...
      html: `<p>OTP Type: ${otpType}</p><p>OTP: <strong>${otp}</strong></p><p>Verify here: <a href="${link}">${link}</a></p>`,
      templateParams: {
        to_email: externalUser.email,
        subject: `${otpType} verification OTP`,
        otpType,
        otp,
        verifyLink: link,
...
router.get("/:kycId/status", async (req, res, next) => {
  try {
...
registerSchemaDispatchRoutes(router, {});
