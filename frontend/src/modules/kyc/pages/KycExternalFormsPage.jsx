...
function KycExternalFormsPage() {
  ...
  const [managerChain, setManagerChain] = useState(null);
  ...
  const loadManagerChain = useCallback(async (managerId) => {
    if (!managerId) {
      setManagerChain(null);
      return;
    }
    try {
      const res = await api.get(`/kyc/approval-chain-preview/${managerId}`);
      setManagerChain(res.data || null);
    } catch {
      setManagerChain(null);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadDispatches();
    loadUsers();
  }, [tenantContextId, loadExternalUsers, loadSchemaModules, loadChainPreview, loadDispatches, loadUsers]);
  ...
  const sendFormDispatches = async (event) => {
    ...
    if (customRaw) {
      approvalChainUserIds = customRaw
        ...
        .map((x) => x.trim())
        .filter(Boolean);
    } else if (selectedManager && managerChain && managerChain.chain?.length) {
      const orderedChain = formDispatch.approvalChainOrder === "REVERSE"
        ? managerChain.reversedChain
        : managerChain.chain;
      approvalChainUserIds = orderedChain.map((u) => u._id);
    }
  ...
  return (
    ...
    >
      ...
      {chainPreview?.defaultOrder?.length ? (
        <p className="small-note">
          <strong>Your default chain ({formDispatch.approvalChainOrder === "REVERSE" ? "top → manager" : "manager → top"}):</strong>{" "}
          {(formDispatch.approvalChainOrder === "REVERSE" ? chainPreview.reversedOrder : chainPreview.defaultOrder)
            .map((u) => u.empCode || u.email || u.name)
            .join(" → ")}
        </p>
      ) : (
      ...
      <form className="stacked-form" onSubmit={sendFormDispatches} style={{ maxWidth: "720px" }}>
        ...
        <label className="stacked-label">
          ...
          <select
            ...
            onChange={(e) => {
              setFormDispatch((p) => ({ ...p, approvalChainOrder: e.target.value }));
              setSelectedManager("");
              setManagerChain(null);
            }}
          >
        ...
        {formDispatch.approvalChainOrder === "DEFAULT" && (
          <label className="stacked-label">
            Select base manager
            <select
              ...
              onChange={(e) => {
                ...
                loadManagerChain(e.target.value);
              }}
            >
              <option value="">Use my chain (default)</option>
              {users
                .filter((u) => u.email !== "superadmin@example.com" && u.status !== "DISABLED")
                .map((u) => (
        ...
        {managerChain && managerChain.chain?.length > 0 && (
          <p className="small-note">
            <strong>Approval chain ({formDispatch.approvalChainOrder === "REVERSE" ? "top → manager" : "manager → top"}):</strong>{" "}
            {(formDispatch.approvalChainOrder === "REVERSE" ? managerChain.reversedChain : managerChain.chain)
              .map((u) => u.empCode || u.email || u.name)
              .join(" → ")}
          </p>
        )}
      ...
      <p className="small-note">
        You see rows you created, where you approve, or all tenant dispatches if you have workflow tracking access. The per-dispatch activity audit trail opens from the Activity log below.
      </p>
      <DataTable
        columns={[
