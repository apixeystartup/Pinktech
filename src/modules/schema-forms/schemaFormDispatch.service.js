...
async function loadUsersMap(userIds, tenantId) {
  ...
  const users = await User.find(query).select("name email empCode").lean();
...
async function createDispatchesForRecipients({
  ...
  const userMap = await loadUsersMap([...chain, dispatchedByUserId], tenantId);
...
async function afterSubmissionNotify({ dispatch, moduleName }) {
  ...
  const userMap = await loadUsersMap([...chain.map(String), String(dispatch.dispatchedByUserId)], dispatch.tenantId);
...
async function afterApprovalStepNotify({ dispatch, moduleName, stepIndex, actorUserId }) {
  ...
  const userMap = await loadUsersMap([...chain, String(actorUserId || ""), String(dispatch.dispatchedByUserId || "")], dispatch.tenantId);
...
async function afterFullyApprovedNotify({ dispatch, moduleName }) {
  ...
  const userMap = await loadUsersMap([...chainIds, String(dispatch.dispatchedByUserId)], dispatch.tenantId);
