...
import { createModule, getFormResponses, getModules, seedClientForm, updateModule } from "../api";
...
function stripBuilderMetaForApi(doc) {
  const next = { ...doc, schema: { ...doc.schema, fields: doc.schema.fields.map(({ _builderId, ...f }) => f) } };
  return next;
}
...
function AdminPage() {
  ...
  const publicUrl = useMemo(() => (selectedId ? `${window.location.origin}/forms/${selectedId}` : ""), [selectedId]);
  ...
  return (
    <Layout title="Form editor" subtitle="Design questions, publish when ready, and review responses in one place.">
            ...
            <tbody>
              {responses.map((item) => (
                <tr key={item._id}>
                  ...
                  <td>
                    {item.pdfDownloadUrl ? (
                      <a className="link" href={item.pdfDownloadUrl} target="_blank" rel="noreferrer">
                        Download PDF
                      </a>
                    ) : (
                      "-"
