...
function DynamicFormRenderer({ moduleDoc }) {
  ...
  const preAgreementText =
    settings.preAgreementText ||
    "I agree not to disclose any data shared during this process.";
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const result = await submitForm({
        ...
        data: values,
        agreements: {
          pre: { accepted: true, text: preAgreementText, signatureDataUrl: preSignatureDataUrl, acceptedAt: now },
          post: { accepted: true, text: postAgreementText, signatureDataUrl: postSignatureDataUrl, acceptedAt: now }
        },
        [honeypotName]: ""
      });
    ...
    <div>
      {phase === "pre" ? (
        <section className="agreementCard glassPanel">
          ...
          <label className="optionLabel">
            <input className="inputGlossy" type="checkbox" checked={preAccepted} onChange={(e) => setPreAccepted(e.target.checked)} />
            I agree
          </label>
          <label htmlFor="pre-signature">Upload Signature Image</label>
          <input id="pre-signature" type="file" accept="image/*" onChange={(e) => fileToDataUrl(e.target.files?.[0], setPreSignatureDataUrl)} />
          <button
            type="button"
            className="ioButton"
            onClick={() => {
      ...
      {phase === "form" ? (
        <form className="grid formSurface" onSubmit={onSubmit}>
          {fields.map((field) => (
            <div key={field.key} className={field.ui?.width === "half" ? "field formFieldWrap" : "field full formFieldWrap"}>
          ...
          <div className="full agreementCard glassPanel">
            ...
            <label className="optionLabel">
              <input className="inputGlossy" type="checkbox" checked={postAccepted} onChange={(e) => setPostAccepted(e.target.checked)} />
              I agree
            </label>
            <label htmlFor="post-signature">Upload Signature Image</label>
            <input id="post-signature" type="file" accept="image/*" onChange={(e) => fileToDataUrl(e.target.files?.[0], setPostSignatureDataUrl)} />
          </div>
      ...
      {statusMessage ? <div className={statusType === "error" ? "alert alert--error full" : "alert alert--success full"}>{statusMessage}</div> : null}
      {pdfUrl ? (
        <p className="hint">
          Submission PDF:{" "}
          <a className="link" href={pdfUrl} target="_blank" rel="noreferrer">
            Download
          </a>
        </p>
      ) : null}
      ...
      <div className="optionGroup">
        {(field.options || []).map((opt) => (
          <label key={opt} className="optionLabel optionPill">
            <input type="radio" name={field.key} checked={value === opt} onChange={() => onChange(opt)} />
            {opt}
          </label>
        ))}
      </div>
